...
while not pare:
    print(mesa)
    cartaescolhida=try_input("Qual posição a sua carta escolhida está?")
    if mesa ==1:
    ...
    while movimentos==[]:
        cartaescolhida= try_input('Vish, carta inválida. Escolha outra:')
        movimentos= lista_movimentos_possiveis(mesa, cartaescolhida)
    
    if movimentos==[1,3]:
        anterior=cartaescolhida-1
        terceira_anterior=cartaescolhida-3
        posicao_escolhida= try_input("Duas possibilidades! {} ou {}?".format(anterior, terceira_anterior))
        while posicao_escolhida!=anterior and posicao_escolhida!=terceira_anterior:
            posicao_escolhida= try_input("Inválido. Só duas possibilidades! {} ou {}?".format(anterior, terceira_anterior))
